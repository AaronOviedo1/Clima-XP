...
- [x] **Fase 2 — Clientes y Rentas (CRUD + estados + pagos)**: CRUD de clientes con normalización E.164 y detección de duplicados; formulario de renta con unidades filtradas por disponibilidad (revalidación transaccional anti-doble-apartado), total en vivo, regla de calentones 3+, descuentos con nota; tarifa de domicilio por km ($89–$610, tabla real) con sugerencia y override; flujo de estados (con efectos sobre inventario) y registro de pagos/anticipos/saldo. Verificado contra Supabase (8/8 reglas). **Edición de renta** (`/rentas/[id]/editar`, mismo `RentaForm` con prop `edicion`): editable en cualquier estado (`ESTADOS_EDITABLES` en `lib/rentas.ts`); con equipo en la calle las unidades quedan bloqueadas (solo fechas/datos/cargos); revalidación de disponibilidad excluye la propia renta; snapshots de precio recalculados al guardar; pagos y estado no se tocan desde el formulario. Verificado contra Supabase (7/7).
  ...
  - **Cotizaciones**: una cotización es una `Renta` con `estado = COTIZADA` (sin modelo ni migración nuevos) y **no aparta equipo** (`apartaInventario()` en `lib/rentas.ts`, misma regla que `ESTADOS_ACTIVOS`). Se captura con el mismo `RentaForm` en `modo="cotizacion"` (`/rentas/nueva?modo=cotizacion`, con su pop-up interceptado): sin anticipo, **dirección opcional** (se guarda `""`, la columna es NOT NULL) y **cliente opcional mientras no se guarde**. Dos salidas: **"Ver"** arma la hoja sin tocar la BD, y **"Guardar"** la deja lista para convertirla en renta. Consecuencias de que no aparte, todas verificadas: `crearRenta` **no rechaza** cotizar equipo ya apartado (devuelve `aviso`, campo nuevo de `RentaActionResult`, y la UI lo muestra como advertencia), `editarRenta` tampoco revalida, y en cambio **`cambiarEstadoRenta` sí revalida al pasar COTIZADA → CONFIRMADA** — ese es el momento en que el equipo se aparta de verdad, y ahí también se exige la dirección. Una COTIZADA no genera cobro (`ESTADOS_SIN_COBRO` en `totalesDeRenta`: saldo 0, como CANCELADA), no suma al total por semana de `/rentas`, y ya no aparece en "entregas de mañana" del dashboard. Verificado contra Supabase (20/20).
  - **Hoja de cotización (imagen)**: `GET /api/cotizacion` devuelve un PNG con `next/og` (satori, sin dependencias nuevas) — `?renta=<id>` para una guardada (usa los snapshots de `RentaUnidad`) o `?u=<ids>&i=&f=&dom=&km=&desc=&cli=` para una que no se guardó (precios vigentes vía `calcularRenta`). **La URL nunca lleva importes**: solo qué se cotiza, para que la hoja no se pueda falsificar. Los datos los arma `lib/cotizacion.ts` y el dibujo vive en `app/api/cotizacion/documento.tsx`: ahí **sí** hay hex sueltos (satori no ve Tailwind ni las CSS vars, y un PNG no tiene modo oscuro) y el **alto se calcula a mano** porque satori no pagina — si se agregan bloques hay que actualizar `altoDocumento` o la hoja se corta. La ruta **no debe terminar en `.png`**: el matcher de `proxy.ts` exime `.png` (los iconos de la PWA se piden sin cookies) y la hoja quedaría pública; además lleva guard `esAdmin`. Fuentes en `public/fuentes/Manrope-*.ttf` (leídas con `fs`, no por URL: `.ttf` sí pasa por el proxy). Compartir (`cotizacion-compartir.tsx`) **baja el PNG al abrir el diálogo, antes del tap**: iOS invalida el gesto si `navigator.share` va después de un `await`, y `wa.me` no adjunta archivos — la imagen llega a WhatsApp por la hoja de compartir del sistema.
