...
**Pop-ups (rutas interceptadas)**: desde la lista, una renta abre en pop-up sobre el fondo; una carga directa de la URL cae en la pantalla completa. Las dos vistas comparten el mismo componente (`renta-detalle` / `renta-editar` / `renta-nueva`, con prop `enModal`), y el pop-up vive en la ranura `@modal` (`(.)rentas/[id]`, `(.)rentas/[id]/editar`, `(.)rentas/nueva`).
