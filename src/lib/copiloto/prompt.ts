...
export function seccionAccionesRecientes(lista: AccionReciente[]): string {
  const lineas = lista.length
    ? lista.map(
        (a) =>
          `- ${HORA_HERMOSILLO.format(a.decididoEn)} · ${a.estado} · ${a.titulo}${a.resultado ? ` (${a.resultado})` : ""}${a.enlace ? ` → ${a.enlace}` : ""}`,
      )
    : ["(ninguna)"];
  return [
