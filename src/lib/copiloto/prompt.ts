...
function reglas(conAcciones: boolean): string {
...
const DESCRIPCION_ROL: Record<ContextoCopiloto["rol"], { base: string; acciones: string }> = {
  ...
  REPARTIDOR: {
    base: "repartidor: NO tiene acceso a montos, saldos, reportes ni historial de clientes, y sus tools no los traen. Si pregunta por dinero o reportes, dile que eso lo consulta el administrador.",
    acciones:
      " Puede proponer marcar En ruta, Entregado y Recogido; no pagos, cancelaciones, cotizaciones ni inventario (eso lo hace el administrador).",
  },
};
...
export function systemPrompt(
  ...
  return [
    { type: "text", text: reglas(conAcciones), cache_control: { type: "ephemeral" } },
